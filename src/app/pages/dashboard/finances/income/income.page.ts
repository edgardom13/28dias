...
export class IncomePage implements OnInit, OnDestroy {
  ...
  // ✅ Recargar datos al entrar a la página (para ver ventas nuevas)
  ...
  async loadData() {
    ...
    try {
      this.transactionsSubscription = this.transactionsService.transactions$.subscribe((transactions: Transaction[]) => {
        this.transactions = transactions.filter(t => t.type === 'income');
        this.applyFilters();
      });

      await Promise.all([
        this.transactionsService.getTransactions(this.businessId, 'income'),
        this.loadStats(),
  ...
  async loadCategoryData() {
    try {
      this.transactionsByCategory = await this.transactionsService.getTransactionsByCategory(this.businessId, 'income');
    } catch (error) {
  ...
  async deleteTransaction(transaction: Transaction) {
    const confirmed = confirm(`¿Estás seguro de eliminar este ingreso de ${this.formatCurrency(transaction.amount)}?`);
    if (!confirmed) return;
  ...
  getCategoryPercentage(category: string): number {
    if (this.stats.totalIncome === 0) return 0;
    const categoryData = this.transactionsByCategory.find(c => c.category === category);
    return categoryData ? (categoryData.amount / this.stats.totalIncome) * 100 : 0;
  }
}